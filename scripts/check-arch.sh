...
# ═══════════════════════════════════════════════════════════════════════════
# 检查 #10: 寄存器合成严禁泄露 (AR-01)
# 规则: _drv.c 中禁止直接对 Config 参数进行数学运算（*/）或位移（<< >>）
#       必须在 _ll.c 中封装为专用的 Compose / Configure 函数
# ═══════════════════════════════════════════════════════════════════════════

section_header 10 "寄存器合成严禁泄露 (AR-01)"

for f in $DRV_FILES; do
    # 查找包含 Config 且包含明确的算术( / , * )或位移( << , >> )或硬编码掩码( & 0x, | 0x )操作符的行
    grep -nE 'Config->.*( / | \* | << | >> | \& 0x| \| 0x)|( / | \* | << | >> | \& 0x| \| 0x).*Config->' "$f" 2>/dev/null | \
        grep -v '^\s*//' | grep -v '^\s*\*' | \
        while IFS=: read -r lineno content; do
            report_violation 10 "$(basename "$f")" "$lineno" \
                "Driver 层违规进行算术/位移计算: $(echo "$content" | sed 's/^[[:space:]]*//')" \
                "将 Config 的数学计算与寄存器组合提取到 *_ll.c 的独立函数中"
        done
done
