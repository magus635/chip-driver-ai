...
- **约束**：
  - **判定准则（LL 文件拆分）**：
    1. **`static inline` (`*_ll.h`)**：仅限于单行寄存器读写、简单的位掩码操作、副作用封装（W1C/RC_SEQ）。
    2. **强制使用 `*_ll.c`**：
       - **寄存器合成 (Composition)**：任何需要根据配置结构体（`ConfigType`）合成寄存器值的逻辑（如 `SPI_LL_ComposeCR1`）。严禁在 `_drv.c` 中直接进行位偏移拼接逻辑（AR-01）。
       - **复杂时序/校准**：超过 3 个步骤的硬件初始化或校准序列。
       - **代码膨胀**：单个逻辑实现超过 10 行代码。
...
## 7. 异步传输与回调模式 (Async & Callback Pattern)

为了支持高性能、非阻塞的数据传输，系统遵循以下集成范式：

1.  **通用服务注册**：
    - 通用模块（如 DMA, Interrupt Manager）必须提供 `pCallback` 和 `pContext` 注册机制。
    - 回调函数应尽量轻量，仅负责状态翻转或事件通知。

2.  **驱动层闭环**：
    - 驱动层（如 SPI, CAN）在启动 DMA 时，必须负责注册自身的内部静态回调（Static Callback）。
    - 驱动层必须通过 `pContext` 传入实例编号，以在回调中定位具体的外设状态机。

3.  **状态机隔离**：
    - 在异步传输期间，驱动状态必须标记为 `BUSY`。
    - 回调触发是清除 `BUSY` 状态的唯一法定路径（除非发生 Timeout 或 Error）。